...
\subsection{1. 하트의 메시지(HART'S
MESSAGE)}\label{uxd558uxd2b8uxc758-uxba54uxc2dcuxc9c0harts-message}
...
\emphksans{법의 개념(The Concept of Law)}의 핵심 사상들이다. 그러나 다른
...
\subsection{2. 사회적 구성물로서의 법(LAW AS A SOCIAL
CONSTRUCTION)}\label{uxc0acuxd68cuxc801-uxad6cuxc131uxbb3cuxb85cuxc11cuxc758-uxbc95law-as-a-social-construction}
...
데리다(Derrida)는 ``\emphksans{il n'y a pas de hors-texte}''(텍스트 바깥은
...
구성물이라고 말할 때, 그것은 어떤 것들이 그렇지 \emphksans{않은} 방식에서
...
사고(thinking)하고 행위(acting)함으로써 만들어낸 것이다.\footnote{See
  eg. John Searle, \emphksans{The Construction of Social Reality} (Allen
  Lane, 1995); and Neil MacCormick, \emphksans{Institutions of Law: An Essay
...
일이기에 굳이 강조할 필요가 없다. \footnote{Cf. Ian Hacking, \emphksans{The
...
타당할 것이다\ldots{}\footnote{Cicero, \emphksans{De Re Republica} III. xii.
...
드물지만, 그 중 일부에 동의하는 이들은 여전히 많다.\footnote{Perhaps
  John Finnis comes closest, in his \emphksans{Natural Law and Natural
...
것이다.\footnote{Ronald Dworkin, \emphksans{Taking Rights Seriously} (Harvard
  University Press, 1978), chap.~4; Ronald Dworkin, \emphksans{Law's Empire}
  (Harvard University Press, 1986), chaps. 2--3.} 드워킨의 설명에
따르면, 도덕적 원칙들에 의해 \emphksans{정당화되는 대상}은 사회적으로 구성된
...
내용들과 사람들이 그것을 정당화라고 \emphksans{믿었거나}, \emphksans{수용했거나},
\emphksans{주장했기 때문}이라고 말하지 않는다. 그는 그 내용들에 \emphksans{실제로
...
\emphksans{법실증주의자}(legal positivist)라고 부르며, 사회구성주의자(social
...
규범(norm)이 적어도 하나는 포함된다고 보았다.\footnote{Hans Kelsen,
  \emphksans{Pure Theory of Law} (Max Knight tr., 2nd edn., University of
...
않았다. 하지만 그는, 그들이 만들어낸 산물을 \emphksans{법으로서(as law)}
...
하트(Hart)는 켈젠의 이러한 견해 역시 거부한다.\footnote{See below,
  ...
  the Unity of Law' in H. L. A. Hart, \emphksans{Essays in Jurisprudence and
...
\subsubsection{\texorpdfstring{(i) \emphksans{법, 규칙, 관행} (Law, Rules,
and
...
않았다.''\footnote{Michel Foucault, `Truth and Power' in his
  \emphksans{Power/Knowledge: Selected Interviews and Other Writings,
...
위협(threats)은 사람들에게 어떤 행위를 하도록 \emphksans{강제(oblige)}할 수는
있지만, \emphksans{의무(obligation)}를 부여할 수는 없다. 결국 주권 이론에서
...
규칙들의 \emphksans{결합(union)}이다. 즉, 인간의 행위를 지도하기 위해
의무(duty)를 부과하거나 권한(power)을 부여하는 \emphksans{1차 규칙(primary
rules)}과, 1차 규칙을 식별하고(identification), 변경(alteration),
집행(enforcement)하기 위한 \emphksans{2차 규칙(secondary rules)}의 결합이다.
이 2차 규칙들 중에서도, 궁극적인 \emphksans{승인 규칙(rule of recognition)}은
...
이 요구를 충족시키지 못한다.\footnote{See Ronald Dworkin, \emphksans{Taking
  Rights Seriously} (rev. edn., Harvard University Press, 1978) 48--58;
  Joseph Raz, \emphksans{Practical Reason and Norms} (2nd edn., Oxford
...
아니라, 자기 행위의 \emphksans{정당화로서(as a justification)} 제시될 수도
...
하트는 이 책의 \emphksans{후기(Postscript)}에서 이러한 비판들에 대응하기 위해
자신의 설명을 한정한다. 그는 이제 모든 규칙이 실천 규칙(practice
rules)은 아니라고 인정하지만, \emphksans{관행적 규칙(conventional rules)}은
...
그러나 여기서 \emphksans{mere}(단지)라는 표현은 삭제해야 할 것이다.
...
이러한 수정을 감안하더라도, 또 다른 문제가 남는다.\footnote{For other
  ...
  `Positivism and Conventionalism' (1999) 12 \emphksans{Canadian Journal of
  Law and Jurisprudence} 35; and Julie Dickson, `Is the Rule of
  Recognition Really a Conventional Rule?' (2007) 27 \emphksans{Oxford
...
일반적으로 \emphksans{공공재(public goods)}에 관한 규칙, 즉
...
가능한 다른 설명들도 있다.\footnote{For example: Joseph Raz,
  \emphksans{Practical Reason and Norms}; Frederick F. Schauer, \emphksans{Playing
  by the Rules: A Philosophical Examination of Rule-based
  Decision-making in Law and in Life} (Oxford University Press, 1993);
  Andrei Marmor, \emphksans{Social Conventions}: \emphksans{From Language to Law}
  (Princeton University Press, 2009); Scott J. Shapiro, \emphksans{Legality}
  (Harvard University Press, 2011). A distinction between rules and
  standards is drawn in Henry M. Hart and Albert Sacks, \emphksans{The Legal
...
\subsubsection{\texorpdfstring{(ii) \emphksans{규칙의 적용 범위(The Reach of
...
그것들이 법체계와 \emphksans{유사하다}는 점은 누구도 부정하지 않는다.) 그러면
...
\emphksans{수 있는} 것뿐 아니라, 실제로 재산(property), 계약(agreements),
...
기준이라는 것은 있을 수 없다. 법은 특정한 \emphksans{유형(kind)}의 규범
체계(normative system) 속에서 일정한 역할을 수행하는 규칙(rules)인
것이다. 그리고 이 규범 체계는 부분적으로 그 \emphksans{내용(content)}에 의해
구별된다.
...
`원칙(principles)'이 그러한 것이라 한다.\footnote{A distinction between
  rules and principles is drawn in Ronald Dworkin, \emphksans{Taking Rights
...
\emphksans{주변적} 현상은 아니다. 이는 모든 법체계와 그 안의 모든 규칙의
...
\emphksans{함께(along with)} 어떻게 작동하는지를 보여줌으로써 설명된다.
...
\emphksans{단순히} 규칙의 문제라고 말한 적이 없다. 또한 규칙이 \emphksans{모든}
...
\subsection{3. 법과
권한 (LAW AND POWER)}\label{law-and-power-uxbc95uxacfc-uxad8cuxd55c}
...
\subsubsection{\texorpdfstring{(i) \emphksans{법에서의 분업(The Division of
Labour in
...
갈등(conflict)과 불일치(disagreement)에도 관한 것이다.\footnote{On other
  aspects of this theme see Jeremy Waldron, \emphksans{Law and Disagreement}
...
때문이다.\footnote{Ronald Dworkin, \emphksans{Law's Empire} 34.}
\end{quote}
...
\emphksans{보편적으로 좋은 것}도 아니고, \emphksans{무조건 좋은 것}도 아니다.
...
\subsubsection{\texorpdfstring{(ii) \emphksans{강제와 권한(Coercion and
...
그 목적이 있다. 제재는 법의 \emphksans{플랜 B}다. \emphksans{플랜 A}는 법의
대상자(subjects)들이 추가적인 감독 없이도 법에 자발적으로 따르도록 하는
것이다. 지시가 필요하지만 강화 동기(reinforcing motivation)는 필요하지
않은 경우에는 제재 없는 법이 그리 드문 것이 아니다. 예컨대 \emphksans{미국
...
수단으로서 수행하는 \emphksans{모든} 주요 기능을 고려하고자 한다면, 여기서
...
일정한 \emphksans{분리(disjunction)}가 존재한다고 강하게 주장한 것으로
...
morals).''\footnote{H. L. A. Hart, `Positivism and the Separation of Law
  and Morals' (1958) 71 \emphksans{Harvard Law Review} 593, at 601 n.~25.}
우리는 이미 위의 3 (i)에서, 법이 반드시 지배받는 인구가 실제로 승인하는
도덕적 가치들을 반영할 필요는 없다는 이유를 살펴보았다. 하지만 그들이
\emphksans{마땅히} 따라야 할 도덕적 가치들과는 어떠한가? 하트는 이 경우에도
...
못했다.\footnote{See John Gardner, `Legal Positivism: 5½ Myths', chap.~2
  of his \emphksans{Law as a Leap of Faith} (Oxford University Press, 2012);
  and Leslie Green, `Positivism and the Inseparability of Law and
  Morals' (2008) 83 \emphksans{New York University Law Review} 1035.} 법과
...
\subsubsection{\texorpdfstring{(i) \emphksans{법의 목적 (Law's
Purpose)}}{(i) 법의 목적 (Law's Purpose)}}\label{i-uxbc95uxc758-uxbaa9uxc801-laws-purpose}

법은 단순한 규칙의 체계가 아니라, 다양한 목적을 수행하는 체계이다.
토마스 아퀴나스는 법이 \emphksans{총체적} 목적(\emphksans{overall} purpose)을
가진다고 보았는데, 그는 이를 ``공공선을 위한 이성적 명령(an ordinance of
reason made for the common good)''이라고 정의하였다.\footnote{\emphksans{Summa
...
강제력을 허용하기 위해 제정된다는 견해들이 있다.\footnote{Guiding
  conduct: Lon L. Fuller, \emphksans{The Morality of Law} (rev. edn., Yale
  University Press, 1969); coordinating activity: John Finnis,
  \emphksans{Natural Law And Natural Rights} (Oxford University Press, 1980);
  doing justice: Michael Moore, `Law as a Functional Kind,' in R. P.
  George ed., \emphksans{Natural Law Theory: Contemporary Essays} (Oxford
  University Press, 1992) 221; licensing coercion: Ronald Dworkin,
  \emphksans{Law's Empire} 93.} 이러한 주장은 법에 대한 이상적 제안이 아니라,
\emphksans{구성적 목적(constitutive aims)}에 대한 주장으로 이해되어야 한다.
...
통합된 법률 체계라도 여전히 법체계로 간주될 수 있다. 이는 \emphksans{어떤
것을 지향한다(aim)}는 것이 반드시 그것을 \emphksans{성취한다(succeeding)}는
...
말했다.\footnote{Hans Kelsen, \emphksans{General Theory of Law and State} (A.
...
있고, 혹은 법이 그러한 목적들에 의해 \emphksans{식별될 수 있다}(identified)는
...
\subsubsection{\texorpdfstring{(ii) \emphksans{법과 정의 (Law and
...
없다. 그런데 하트는 이 일관성(constancy) \emphksans{자체가(itself)} 일종의
...
결국, 정의\emphksans{의 형식}(\emphksans{the form of} justice)을 가진 모든 것이
반드시 정의의 \emphksans{한} 형식(\emphksans{a} form of justice)인 것은 아니다.
...
않다.\footnote{Following John Gardner, `The Virtue of Justice and the
  Character of Law', chap.~10 of his \emphksans{Law as a Leap of Faith}.}
예를 들어, ``남성과 여성은 동등한 가치의 노동에 대해 동일한 보수를
받아야 한다''는 규범은 정의 규범이다. 반면, ``남성과 여성은 동등한
가치의 노동에 대해 서로 다른 보수를 받아야 한다''는 규범은 \emphksans{부정의
...
정의인지, 부정의인지, 또는 둘 다 아닌지 여부를 \emphksans{형식}만으로는
판단할 수 없다는 점이 분명해진다.
...
법을 \emphksans{모든(all)} 경우들에 일관되게 적용해야 할 이유가 있는가?
여기서는 \emphksans{법률 없이는 형벌 없다(nulla poena sine lege)}라는 원칙
이상의 것이 필요하다. 그 원칙은 법을 위반\emphksans{하지 않은(have not)}
사람을 처벌하지 말라는 것이지, 법을 위반한 모든 사람을 반드시 처벌하라는
것이 \emphksans{아니다(not)}. 이 조건은 우리의 가설적 사례에서 충족된다.
하지만 그 원칙은 법을 위반\emphksans{한(has)} 모든 사람을 처벌하라고 말하지
않는다. 그렇게 하지 않는 것이 \emphksans{부정의}인가? 만약 그렇다면, 누구에게
...
적용(rule-application)에 주목하게 될 때, 우리는 필연적으로 \emphksans{특정
...
\subsubsection{\texorpdfstring{(iii) \emphksans{법적 유효성과 도덕 원칙
...
근원이 \emphksans{되는(are)} 요소들에 의해 승인(authorized)된다면 법의 근원이
\emphksans{될 수 있다(could)}는 점을 인정한다. 이를 다르게 표현하면, 하트는
...
positivism)'에는 반대한다.\footnote{Hart calls his position `soft'
  ...
  holds that law \emphksans{includes} anything to which law refers.} 그가 이
...
가지는 다음과 같다. 하나는 \emphksans{내용(substance)}과의 대비이고, 다른
하나는 \emphksans{도덕성(morality)}과의 대비다. 이 혼동은 `실질적
...
있다.) 하지만 여기서 물어야 한다: (P2)는 또한 \emphksans{족보}의 테스트(a
test of \emphksans{pedigree})를 진술하는가? 그것은, 해당 기준을 적용하려면
제정된 법의 내용이 아니라, 제정 방식을 살펴보아야 한다는 의미에서는 족보
기준이다. 하지만 그것(P2)이 충족되었는지 \emphksans{여부(whether)}를 알기
위해서는 제정 과정의 \emphksans{도덕적 적정성(moral propriety)}에 대한 판단이
...
\emphksans{용어들(terms)}이 있다고 해서, 그것이 곧 법에 대한 도덕적 기준이
...
`불공정하다(unfair)'고 말할 수 있다면, 그것이 불공정한 \emphksans{근거가
...
도덕에 따라 법을 \emphksans{창출하라}(\emphksans{make} law)는 지시를 내리는 것일
...
뿐 아니라, \emphksans{또한(and)} 그 불확실성에 대한 도덕적 판단이
...
작용할 수 있다.\footnote{Hans Kelsen, \emphksans{Pure Theory of Law} 350--1.}
...
이런 형태로 문제를 자초하지는 않는다.''\footnote{H. L. A. Hart,
  \emphksans{Essays in Jurisprudence and Philosophy} 361.} 그러나
...
권위(authority)를 가질 수 없게 된다고 주장한다.\footnote{Joseph Raz,
  \emphksans{The Morality of Freedom} (Oxford University Press, 1986),
  chap.~2; \emphksans{Ethics in the Public Domain}, chap.~10; and
  \emphksans{Between Authority and Interpretation: On the Theory of Law and
...
해야 할 일을 따르도록 돕는 것이다.\footnote{이는 단지 그들이
  \emphksans{이기적(selfish)} 이익에 부합하는 행동이거나, 자신이
  \emphksans{이유(reason)} 있다고 \emphksans{생각(think)} 하는 행동이 아니라,
...
무엇을 해야 하는지를 \emphksans{파악(figure out)}해야만 한다면, 법은 그에게
...
것이다.\footnote{게다가 이후의 한 논문에서 하트 자신도 이 입장의 몇 가지
  ...
  Authoritative Legal Reasons」, \emphksans{Essays on Bentham} (Oxford
...
validity)을 통해 그러한 초대(invitation)를 발행한다.\footnote{토니
  ...
  Connection between Law and Morality」, (2002) \emphksans{Oxford Journal of
  Legal Studies} 22:489.} 그러나 과연 초대가 정말 필요한가?\footnote{See
  Joseph Raz, \emphksans{Between Authority and Interpretation}, chap.~7.}
...
\subsection{5. 사실(fact), 가치(value),
방법(method)}\label{uxc0acuxc2e4fact-uxac00uxce58value-uxbc29uxbc95method}
...
\subsubsection{\texorpdfstring{(i) \emphksans{법철학(jurisprudence)과
...
\emphksans{선험적(a priori)} 이론과 법에 대한 보통의 지식 사이에 충돌이
...
의해 정의된다''고 말했다\footnote{Hans Kelsen, \emphksans{General Theory of
  Law and State} 178. `Normative' here means `having to do with norms',
  \emphksans{not} `morally appraisive'.}. 그의 기본 생각은, 사회학이 법
제도나 실천을 연구하고 일반화하려면 \emphksans{그것들을 확보하고(with them)}
...
모두 법의 지배의 핵심 요건들이다\footnote{For a review some attempts at
  ...
  `Pitfalls of Measuring the Rule of Law' (2011) 3 \emphksans{Hague Journal
...
있다는 반론이 있을 수 있다\footnote{For a spirited defence of a
  ...
  \emphksans{Naturalizing Jurisprudence: Essays on American Legal Realism and
...
이끌어내려 하고, 때때로 그는 우리가 무엇이라고 \emphksans{말할(say)} 것인지를
묻는 방식으로 \emphksans{그 작업을(that)} 수행한다. 이것이 하트의 법철학을
의미론(semantics)의 한 분야로 만든다고 볼 수 있을까? 하트는 철학의
`언어적 전환(linguistic turn)'에 영향을 받았고, 스스로 그 옹호자라
여겼다\footnote{See Richard Rorty ed., \emphksans{The Linguistic Turn: Recent
...
Ryle)이 전개한 일상 언어철학에 영향을 받았다\footnote{그리고 후기
  ...
  있는 논의는 니콜라 레이시(Nicola Lacey), \emphksans{A Life of H. L. A.
...
언어분석이 \emphksans{거의 없다(little)}는 것이다. 언어는 다양한 기능을
...
Hume)은 정치학이 과학이 될 수 있다고 말했지만, \emphksans{논고(Treatise)}나
\emphksans{탐구(Enquiry)} 어느 곳에서도 단 하나의 실험이나 증명을 제시하지
않았다.) 현대 철학의 수사적 스타일은 하트의 것과 매우 다르며, 하트의
스타일은 다시 벤담(Bentham)의 것과 매우 달랐다. 이러한 차이가 실제
기법의 차이를 얼마나 반영하는지는 분명하지 않다.

\subsubsection{\texorpdfstring{(ii) \emphksans{명료화(clarification)와
...
대한 이론이 아니라, \emphksans{법 그 자체(as such)} 에 대한 이론이라고
...
때문이다. 실제의 묘사는 그에 관한 \emphksans{모든} 사실의 목록이 아니라, 어떤
...
조건을 누리게 된다. 이 지점에서 비판자들이 반박에 나선다\footnote{Among
  ...
  Law' in Stephen Guest ed., \emphksans{Positivism Today} (Dartmouth, 1996)
...
\emphksans{필요하지(need)} 않기 때문이다. 인간 본성이나 사회라는 것의 구조
...
주목할 점은, 첫째로, 문제 삼는 `결함(defect)'은 \emphksans{도덕적}
악(\emphksans{moral} vice)이 아니라, 사회 통제 메커니즘의 기능적
...
않았다고 보는 것은 편협하고 모욕적인 생각이므로, \emphksans{후건부정법(modus
tollens)}에 따라, 모든 사회는 법체계를 가져야 한다. 그리고 어떠한
법철학이든 이러한 사회들을 법체계들\emphksans{로써(as)} 간주하지 않는다면,
...
체계(system)가 아니라 규칙들의 \emphksans{집합}(\emphksans{set} of rules)으로
...
본다\footnote{Ian Brownlie, `The Reality and Efficacy of International
  Law' (1981) 52 \emphksans{British Yearbook of International Law} 1, 7--8.}.
그들은 자신들의 학문분야의 지위에 대해 걱정하는 나머지, 이 장의 핵심이
오히려 국제법을 \emphksans{옹호하고(defend)} 있다는 점은 거의 눈치채지
...
있다\footnote{For a review of some of the issues see the editors'
  introduction to Samantha Besson and John Tasioulas eds., \emphksans{The
...
벌린(Isaiah Berlin)처럼\footnote{Isaiah Berlin, \emphksans{Four Essays on
  Liberty} (Oxford University Press, 1969), and also his `On Value
  Pluralism' (1998) \emphksans{New York Review of Books}, vol.~XLV No.~8.},
...
일원론(nonreductive monism)에도 반대한다\footnote{이 입장에 대한 가장
  지속적인 옹호는 로널드 드워킨(Ronald Dworkin), \emphksans{Justice for
...
것을 \emphksans{입증(demonstrate)}할 수는 없다\ldots{}'' (205).

\subsection{6. 요점(THE POINT)}\label{uxc694uxc810the-point}
...
법이론서(jurisprudence)를 \emphksans{다른(other)} 법이론서에 대해 알기 위해
...
조언서(high-brow \emphksans{amicus curiae} brief)'로서 영향력을 행사한 예는
...
줄어들었다\footnote{이 주제가 응용 도덕철학(applied moral philosophy)의
  ...
  「\emphksans{Political} Political Theory: An Oxford Inaugural Lecture」,
...
중 어느 것도 법철학의 \emphksans{그(the)} 질문이 아니다. 이들은 모두 법철학의
\emphksans{질문들(questions)}일 뿐이다. 그렇다면 이 질문들은 흥미로운가? 물론
...
흥미를 느끼지 못했다\ldots{}''\footnote{David Hume, `My Own Life' in his
  \emphksans{Essays}, \emphksans{Moral, Political and Literary}, E. F. Miller,
...
사람들에게는 분명 흥미를 유발할 수 있다. 법정에서는 `\emphksans{당해 사안의
법(the law)}이 무엇인가'를 아는 것이 `\emphksans{법(law)}이 무엇인가'를 아는
