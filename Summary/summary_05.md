...
* 법의 다양한 특징(형법, 권한 규칙, 입법 연속성 등)을 설명하지 못함
* ‘암묵적 명령’, ‘공무원 중심 규칙’ 등 보완 개념도 실패
* 규칙 개념 없이 법은 설명 불가능
...
* 두 종류의 규칙 필요
  ...
  * **일차 규칙 (primary rules):** 의무를 부과함
  * **이차 규칙 (secondary rules):** 규칙을 인식, 변경, 적용할 수 있는 권한을 부여함
...
* 각각의 결함을 대응하는 이차 규칙들
  ...
  * **판결 규칙 (rules of adjudication)**: 위반 여부를 판단할 권한 부여
...
* **예측 이론 (predictive theory)** 비판
  ...
  * 규칙 위반은 단순히 제재 예측의 근거가 아니라 **정당화(reason)의 근거**
  * 규칙은 \*\*내적 관점 (internal point of view)\*\*에서 이해되어야 함
...
| 도덕 규범도 압력·절차 존재       | 법과 도덕 경계 불분명 | 제도화된 운영과 공적 권한 여부가 핵심 기준              |
...
* 법적 개념들의 내적 측면을 해명하는 데 핵심 도구 제공
...
| C  | **해결·귀결**  | 각 결함에 対하여 ⓐ 승인 규칙 ⓑ 변경 규칙 ⓒ 판결 규칙을 도입하면 결함이 해소되고, 그 총체가 ‘법’임을 귀결 |
...
4. **P4** 각 결함은 논리적으로 **특정 유형의 규칙(이차 규칙)** 도입으로만 해소된다.
   ...
   * P4-c 비효율성 → “판결 규칙”(rules of adjudication)
...
| **R4** ‘도덕규범도 강한 사회적 압력·변경 절차를 가질 수 있는데 왜 법과 구별?’ | 규칙 유형만으로 법·도덕 선긋기 불가             | • Hart는 “적어도 최소한의 이차 규칙의 제도화된 운영(official practice)”을 법의 표지로 제시.<br>• 도덕은 통상 분산된 압력·탈제도화 절차… 정도·기관화 차이가 본질.                     |
| **R5** ‘소규모 호혜 공동체는 이차 규칙 없이도 효과적’                | 결함이 반드시 치명적이진 않다                 | • Hart도 인정: “작은·안정된 공동체” 예외. 그러나 법이론은 *대규모·복합 사회* 설명 목표—“규범적 일반성” 관점에서 여전히 필요조건.                                                |
| **R6** ‘명령-이론 보완(예: 규칙 = 반복 + 제재)으로도 결함 해결 가능’    | “강제 명령”을 더 세분하면 충분               | • 권한 부여·유효성·판결 개념은 제재로 환원 불가(내적 정당화 사유·행위구조 설명 실패).<br>• 명령의 ‘발생 조건’과 ‘발효 절차’를 설명하려면 이미 이차 규칙적 요소 도입이 불가피.                      |
...
* Hart-식 대응은 **기능 분석**과 **내적 관점** 논지를 통해 반론을 흡수·재배치하면서 “1차+2차 결합 모형”의 설명력을 방어한다.
