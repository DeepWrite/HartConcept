...
- Hohfeldian rights analysis: 호펠드(Wesley Hohfeld)의 권리 이론은 법적 권리를 네 가지 기본 지위—청구권(claim-right), 자유권(privilege 또는 liberty), 권한(power), 면책권(immunity)—으로 구분하며, 각 지위는 대응하는 상관자 지위(correlative)와 반대 지위(opposite)를 가진다. 예컨대 청구권은 상대방의 의무(duty)를, 자유권은 상대방의 무권리(no-right)를 전제하며, 이는 법적 관계의 정밀한 분석을 가능하게 한다. 이 이론은 권리를 단일 개념이 아닌 상호구조적 관계로 파악함으로써, 권리담론의 논리적 혼동을 피하고 법적 논증의 명료성을 높이는 데 중요한 기여를 한다. 호펠드의 네 가지 권리 유형은 각각 구체적인 사례를 통해 쉽게 이해될 수 있다. **청구권(claim-right)**은 예를 들어 A가 B에게 임대료 지급을 요구할 수 있는 권리처럼, 타인에게 특정 행위를 요구할 수 있는 권리이다. **특권/자유권(privilege 또는 liberty)**은 A가 자신의 토지에 들어갈 자유처럼, 타인의 간섭 없이 스스로 어떤 행위를 할 수 있는 권리를 의미하며, 타인에게 의무를 발생시키지는 않는다. **권한(power)**은 A가 유언장을 작성하여 상속 구조를 바꿀 수 있는 것처럼, 타인의 법적 지위나 관계를 변화시킬 수 있도록 법적으로 승인된 지위를 말한다. 마지막으로 **면책권/면제(immunity)**은 A가 헌법에 의해 표현의 자유를 보장받아 국가가 이를 제한할 수 없는 경우처럼, 타인이 자신의 법적 지위를 변경할 수 없는 상태를 뜻한다.
...
| Power (권한)             | 법적 지위를 **변경할 수 있는 권한**                    | Liability (책임 있음)      | Disability (무권한)      | A는 B에게 계약을 체결할 권한이 있음   |
...
- Hart 번역 정책: Hohfeldian `power`, Hart의 `legal power`, `power-conferring
  rules`는 원칙적으로 `권한`, `법적 권한`, `권한부여 규칙`으로 통일한다.
  `권능`은 설명용 보조어로만 사용하고, 반복 표제 번역어로 쓰지 않는다.
- 위 Hohfeld 표의 `Duty (의무)`는 청구권의 상관자 지위를 가리키는 한국어
  권리분석 통례이다. Hart 본문에서 source `duty`가 `obligation`과 구별되어
  등장하는 경우에는 정책상 `책무`를 우선하고, `duty-imposing rules`는
  `책무부과 규칙`으로 유지한다.
- German `Gestaltungsrecht`는 `형성권`이다. 이는 Hohfeldian `power`의
  대표적인 사법상 하위사례 또는 인접 개념으로 설명할 수 있지만, Hart의
  `power-conferring rules` 전체와 등치하지 않는다. Hart의 범위에는 계약,
  유언, 재산이전뿐 아니라 재판권, 입법권, 행정적 규칙제정권도 포함된다.
- 한국어 법철학 번역 관행 중 Hohfeldian `power`를 `형성권`으로 옮기는
  선택은 별도 문헌 메모에서 비판적으로 추적한다. 이 Hart surface에서는
  `형성권`을 기본 번역어로 삼지 않는다.
...
15. idea / notion / concept / definition / conception

- idea: 기본값은 `관념`. 전통상 강한 규범적ㆍ초월적 함의가 있는 경우에만
  `이념`을 검토한다. 단순 착안은 `발상`으로 둘 수 있다.
- notion: `개념적 파악`, `통념`, `관념` 중 문맥별로 고르되, `idea`나
  `concept`와 가까운 곳에 함께 나오면 같은 한국어로 평평하게 합치지 않는다.
- concept: `개념`. 제목어, 분석 대상, 의미와 적용 기준의 구분에서는 이
  번역어를 보호한다.
- conception: `구상`, `이해방식`, `관`. 특히 Rawls/Dworkin식
  `concept/conception` 구분에서는 `개념/구상` 또는 `개념/이해방식`으로
  분리한다.
- definition: `정의`. `concept`를 정의문으로 환원하지 않는 Hart식 구분에
  주의한다.
